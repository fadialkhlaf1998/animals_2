...
class APIWEB {
  ...
  static Future<int> getCompanyId() async {
    ...
    // var response = await Dio().post(url + '/api/app-key',data: {"public_key": public_key});
  ...
  static Future<List<PostType>> postType() async {
    ...
    request.body = json.encode({"locale": Store.languageCode});
    request.headers.addAll(headers);
  ...
  static Future<Post?> getPostInfo(int id) async {
    ...
    request.body = json.encode(
        {"id": id, "customer_id": customer_id, "locale": Store.languageCode});
    request.headers.addAll(headers);
